...
public class Main {
	...
	static class Bead {
		...
		public Bead(int redR, int redC, int blueR, int blueC, String sDir) {
			this.redR = redR;
			this.redC = redC;
			this.blueR = blueR;
			this.blueC = blueC;
	...
	static String[] stringDir = { "D", "U", "R", "L" };
	...
	public static void main(String[] args) throws Exception {
		...
		// 정답 출력
		sb.append(bfs(redR, redC, blueR, blueC)).append("\n").append(ans);
		System.out.println(sb);
	}

	// BFS 탐색
	static int bfs(int redR, int redC, int blueR, int blueC) {
		Deque<Bead> q = new ArrayDeque<>();
		boolean[][][][] visited = new boolean[n][m][n][m];

		q.add(new Bead(redR, redC, blueR, blueC, ""));
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int d = 0; d < 4; d++) {
					int[] red = { cur.redR, cur.redC }; // 빨간공 좌표
					int[] blue = { cur.blueR, cur.blueC }; // 파란공 좌표
					...
					if (redFlag) { // 빨간 구슬이 빠진 경우
						if (blueFlag) // 파란 구슬이 빠진 경우, 스킵
							continue;
						ans = cur.history == null ? stringDir[d] : cur.history + stringDir[d];
						...
						q.add(new Bead(red[0], red[1], blue[0], blue[1], cur.history + stringDir[d]));
					}
				}
			}
